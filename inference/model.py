...
class ModelArgs:
    ...
    # 每个token激活的专家数（6/64稀疏激活）
    n_activated_experts: int = 6
    # 专家分组数（1表示不分组）
    n_expert_groups: int = 1
    # 有限分组数（路由限制）
    n_limited_groups: int = 1
    # 路由评分函数（分配token给专家的方法）
...
class ParallelEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if world_size > 1:
            ...
            # 全局索引变为局部索引：说白了就是变为当前GPU中索引
            x = x - self.vocab_start_idx
            # 将掩码中为True的位置对应的x元素设置为0
            x[mask] = 0
        # 进行查表处理，将token索引转换为词向量
...
class MLA(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        # 等价于：1 / sqrt(qk_head_dim)，缩放注意力分数，防止梯度消失或爆炸
    ...
    def forward(
        ...
        if self.q_lora_rank == 0:
            # 2048 -> 16 * 192 = 3072
            # 将输入特征x通过查询投影线性层映射为注意力机制中的查询向量Q
            q = self.wq(x)
        # 使用LoRA（低秩适应）风格的降维-升维投影
        else:
            q = self.wq_b(self.q_norm(self.wq_a(x)))

        # 3072 -> 16 / 1 , 192
        q = q.view(bsz, seqlen, self.n_local_heads, self.qk_head_dim)
        # 将查询向量Q按最后一个维度分割为不使用位置编码的部分q_nope和使用位置编码的部分q_pe
...
class MLP(nn.Module):
    ...
    def __init__(self, dim: int, inter_dim: int):
        ...
        """
        super().__init__()
        # 列并行：拆分W矩阵的列到各个GPU，并行计算，最后结果左右拼接
        self.w1 = ColumnParallelLinear(dim, inter_dim)
        # 行并行：拆分W矩阵和输入X矩阵的行，并行计算，最后结果相加
...
class Gate(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        # 将其包装为可训练参数，会出现在model.parameters()中，参与梯度下降优化
        self.weight = nn.Parameter(
            # 创建一个空的权重矩阵，用于将输入映射到专家的选择上
